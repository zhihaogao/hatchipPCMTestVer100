...
namespace Arcone.Component.Tester.Function.Hatchip
{
    public partial class TESTDISP
    {
        ...
        [CharacteristicParameter(Code = "X1", Name = "Sweep X1(V)", Desc = "start value", DefaultValue = "0",
            ControlType = ParameterLayoutControlType.TextBox, DataType = ParameterLayoutDataType.Double)]
        [CharacteristicParameter(Code = "X2", Name = "Sweep X2", Desc = "stop value", DefaultValue = "1",
            ControlType = ParameterLayoutControlType.TextBox, DataType = ParameterLayoutDataType.Double)]
        [CharacteristicParameter(Code = "XStep", Name = "Sweep Step", Desc = "step", DefaultValue = "0.01",
            ControlType = ParameterLayoutControlType.TextBox, DataType = ParameterLayoutDataType.Double)]
        ...
        [CharacteristicParameter(Code = "Compliance0", Name = "Force COMP(I)", Desc = "Force SMU compliance", DefaultValue = "100E-3",
            ControlType = ParameterLayoutControlType.TextBox, DataType = ParameterLayoutDataType.Double)]
        ...
        [CharacteristicParameter(Code = "Para2", Name = "AtVIR", Desc = "get the V IR", DefaultValue = "0",
           ControlType = ParameterLayoutControlType.TextBox, DataType = ParameterLayoutDataType.Double)]
        ...
        public void characteristics4(bool IsTestingMode)
        {
            //step1.设置图表样式
            ...
            if (IsTestingMode)
            {
                ...
                _ = HP4142.Send($"WV{forceSmu},1,0,{x1},{x2},{No_step},{forceSmu_COMPLIANCE}");
                ...
                double atV = Parameters.Para2;
                if (!DoubleUtil.IsZero(atV) && result.Any())
                {
                    var Y1 = result.Select(c => c.Y).ToArray();
                    double i = ArconeMath.FindY(x1, x2, xstep, No_step, atV, ref Y1, 0);
                    double R = atV / (i + 1E-15);
                    Vars[0] = i;
                    Vars[1] = atV;
                    Vars[2] = R;
                }
